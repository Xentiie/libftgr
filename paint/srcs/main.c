...
#define COLOR_PICKER_RECT_SIZE 30
	...
	for (U64 i = 0; i < array_len(cols); i++)
	{
		...
		ftgr_wdrawer_fill_rect(w, cols[i]);
	...
	ftgr_wdrawer_fill_rect(selected_color_widget, selected_color);
	ftgr_wdrawer_draw_rect(selected_color_widget, COL_BLACK);
	ftgr_add_widget(selected_color_widget, win->w_root);

	{
		bitmap_img = ftgr_load_png("../centrale/font_bitmap_black.png");
		if (bitmap_img == NULL)
			goto no_label;
		...
		ftgr_wdrawer_fill_rect((t_widget *)brush_size_label, COL_WHITE);
...
#define BRUSH_SIZE_LABEL_STR_SIZE sizeof("Brush size: ")
	...
	t_ftgr_img *canvas = ftgr_new_img(ivec2(win->size.x, win->size.y - COLOR_PICKER_RECT_SIZE - 5));
